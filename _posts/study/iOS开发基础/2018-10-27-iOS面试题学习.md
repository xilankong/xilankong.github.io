...
#### 19、Socket的实现原理及 Socket之间是如何通信的
...
1、更加简洁、函数为一等公民，函数式编程

2、有类型推断、optional，更加的安全

3、支持泛型、强大的枚举、强大的模式匹配
...
```
当不希望外部对所定义对象进行修改 复写，比如不希望被更改的父类，不希望被更改的方法
```
...
#### 55.编译选项 whole module optmization 优化了什么

```
编译器对你的代码进行分析的时候，再也不会局限于一个文件当中了，而是整个module。有什么用呢，有了这一特性，编译器可以对你的代码了解得更多，能更好的做好编译工作。比如下面这个例子：
```



#### 56.下面代码中 mutating 的作用是什么
...
#### 59.为什么数组索引越界会崩溃，而字典用下标取值时 key 没有对应值的话返回的是 nil 不会崩溃。
...
#### 60.一个函数的参数类型只要是数字（Int、Float）都可以，要怎么表示。
