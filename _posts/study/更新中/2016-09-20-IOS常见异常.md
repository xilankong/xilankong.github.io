...
profile中无法找到对应的cert啊之类的。

删光证书 profile 重新match



在服务器上信任通过证书使用，不然会  unknown error -1=ffffffffffffffff
...
http://www.jianshu.com/p/2800dfbcdc04
...
出现上面问题，然后自己的match又没有问题的时候，原因就是因为开启了 Xcode 的 automatically manage signing，导致寻找 profile冲突。所以当使用match的时候建议关闭auto



collectionView不执行dataSource协议方法:

虽然过了这么久了，最后还是回答一下为后面的的人解答吧：： UICollectionViewFlowLayout *layOut = [[UICollectionViewFlowLayout alloc] init];要这一句    记得千万不要把UICollectionViewFlowLayout 写成了UICollectionViewLayout 





Xcode 9 通过文件复制进项目的文件老是出现奇怪变异问题，在build phases中重新加一下文件的 complie source
...
IBInspectable 出现未定义等 找不到赋值地方。其实是在xib的 user defined runtime attributes 中定义了，通过这种方式获取
...
[![96](https://upload.jianshu.io/users/upload_avatars/4222928/ab03ddb6-b0c5-4d11-868b-8192df9e833b.jpg?imageMogr2/auto-orient/strip|imageView2/1/w/96/h/96)](https://www.jianshu.com/u/49bca1707f7e)  [Simple_Dev](https://www.jianshu.com/u/49bca1707f7e) [**关注]()2017.01.09 13:29* 字数 445 阅读 530评论 3喜欢 1
