...
https://github.com/Damonvvong/DevNotes/blob/master/Notes/swift_performance.md
...
1. 堆 －－ 一般由程序员分配释放，若程序员不释放，程序结束时    可能由OS回收 
2. 全局区（静态区） －－ 全局变量和静态变量的存储是放在一块 的，初始化的全局变量和静态变量在一块区域，未初始化的全局变量和未初始化的静态变量在相邻的另一块区域。程序结束释放。 
3. 另外还有一个专门放常量的地方。程序结束释放。
...
消息推送的流程必须要经过APNs(Apple Push Notification service).

手机会与苹果APNs服务器有一个长链接

客户端 发送设备的UDID和应用的BundleID 给 APNs服务器 ，苹果生成一个deviceToken 返回客户端。(客户端可以通过token找到用户A)，把deviceToken 和用户信息绑定存库  通过用户的deviceToken 把通知信息发送到APNs，APNs服务器找到对应用户发起通知。
...
2、APNs经过打包和处理产生devicetoken并返回给注册的Device（设备）

3、Device（设备）携带获取的devicetoken发送到我们自己的应用服务器

4、完成需要被推送的Deviceå（设备）在APNs服务器和我们自己的应用服务器的注册
...
**typedef int** (*block类型名称)**(int, int)**; 



声明一个block  void (^myBlock)();   typedef int (^myBlock)(int a,int b) ;

https://blog.csdn.net/u010963948/article/details/49637139
...
对象需要内存管理、基本数据类型不需要内存管理

堆内存需要动态分配、程序员手动管理，现在有ARC
```

19、NSCopying协议、copy、mutableCopy

```
浅复制：不拷贝对象本身，拷贝指向对象的指针，拷贝完成最终还是指向同一对象的内存地址
深复制：直接拷贝对象本身到另一内存地址
...
20、weak,strong,copy,readOnly,assign,nonatomic

```
assign: 基本数据类型，非对象，不需要引用计数

retain 默认: 用于对象，引用计数加 1   当用retain时，会释放旧的对象，将输入对象的索引计数+1，然后将输入对象的值赋予新对象。

copy： 她的setter方法会执行copy操作，拷贝的新对象引用计数为 1 ，然后付给指针，原有对象引用计数不变，  copy常用于NSString

strong： strong是强引用，是ARC新引入的对象变量属性，简单讲strong等同retain，只有在你打开ARC时才会被要求使用；
...
非原子性访问，对属性赋值操作的时候不加锁，不同步，多线程并发访问会提高性能，但是不能保证不出错

atomic

原子性，多线程环境下保证数据安全，属性操作都是原子操作，但是并发性能低
```
...
26、#import 跟 #include、@class有什么区别？＃import<> 跟 #import""又什么区别？

```
#import 可以防止同一个文件被多次包含

@class 是声明操作，并不会包含完整的文件 @class解决循环包含问题

#import <> 包含库的头文件 #import “” 包含文件
```
...
线程是进程中的一个实例，是操作系统可识别的最小执行和调度单位。
