...
masonry 当针对负的offset设置大于等于时要用lessThan
...
1. masonry 当针对负的offset设置大于等于时要用lessThan
...
1. App 播放视频时进入后台让视频继续播放，有两种方法可以实现：
...
2. 获取视频的每秒帧数 `fps` 和 视频的长度 `duration`：
...
5. 禁止 `oc` 文件使用 `ARC`：`-fno-objc-arc`

6. 禁止 `UIButton` 在按下时有动态阴影：
...
11. 模拟器截图时，必须设置分辨率为100%（Simulator > Window > Scale > 100%）

12. *UIView* 中添加定时器时导致内存泄露处理：
...
13. *UITextfiled* 设置 *placehold* 字体颜色：
...
22. *swift* 中将 *Unmanaged<CGImage>* 转化为 *UIImage*
...
26. `AudioServicesPlaySystemSound` 和 `AudioServicesPlayAlertSound` 的区别：
...
27. 当 *UIButton* 即有文字又有图像时，只让图像旋转：
...
28. 调用 *AVPlayer* 的 `seekToTime` 接口时间不对，将
...
29. 使用 `NSDateFormatter.dateFromString` 总是返回 `nil`，原因可能是你的时间格式是`12小时制而你的值是24小时制`，将 `hh:mm:ss` 改成 `HH:mm:ss`即可。

30. `swift` 中结构体与结构体指针的使用，`SMsgAVIoctrlSetPlayVolumeReq` 为结构体， `IOTYPE_USER_IPCAM_AUDIO_PLAY_VOLUME_SETTING_REQ` 为枚举变量：
...
33. [swift 的黑魔法](http://swifter.tips/swizzle/)
...
37. `UIButton` 中的 `文本` 和 `图片` 默认是水平对齐

38. 使用 `单元测试` 时显示 `Module 'CTest' was not compiled for testing`：
    在 `Build Settings` 中 `Enable Testability` 设置为 `YES`

39. `Xcode 8` 控制台输出信息太多解决方法：`Edit Scheme` -> `Run` -> `Arguments`， 在 `Environment Variables` 里边添加 `OS_ACTIVITY_MODE`，值设置为 `disable`
...
41. `lazy` 属性本质上是一个闭包，闭包中的表达式只会调用一次。需要强调的是，虽然这个闭包中捕获了`self`，但是这样做并不会导致循环引用，猜测是 `swift` 自动把 `self` 标记为 `unowned` 了。
...
46. 设置 `UITableView` 中的 `cell` 的`分隔线`铺满整个 `cell`
...
48. 获取 `UILabel` 设置 `text` 后的大小：

    ```
    label.intrinsicContentSize
    ```

49. `UIButton` 添加子视图后无法响应事件的处理：
...
50. 使用 `UISearchController` 导致的黑屏问题，在使用 `UISearchController` 的 `UIViewController` 的 `viewDidLoad` 添加：

    ```
    self.definesPresentationContext = true
    ```

51. 自定义 `UISearchBar` 中的 `UITextField` 无效：必须确保 `UISearchBar` 的 `searchBarStyle` 为 `prominent` ，当其值为 `minimal` 时，自定义 `UITextField` 无效。

52. 子控件（比如按钮）超出父视图部分无法响应事件，在父视图添加以下代码（其思路是遍历父视图的所有子视图，并判断触发事件的点是否在子视图的bounds内如果在就返回这个子视图。）：
...
53. `Xcode` 无法在高于某系统的真机上运行：`This iPhone 6 is running iOS 10.1 (14B55或者14B72), which may not be supported by this version of Xcode`：
    把升级包放在路径：`应用程序（xcode）-右键-显示包内容  /Developer/Platforms/iPhoneOS.platform/DeviceSupport`

54. 在控制器 `A` 中，`push` 展示 控制器 `B`，这时要隐藏控制器 `B` 的返回按钮的文本，需要在控制器 `A` 中添加以下代码，而不是在控制器 `B` 中，因为 控制器 `B` 中的返回按钮实际是控制器 `A` 的：
...
56. 设置导航栏返回按钮图片：在创建导航栏的时候添加以下代码
...
57. `UITableViewCell` 被点击时，`subviews` 消失，这是因为当 `UITableViewCell` 被点击时会改变它内部的 `subviews` 的背景色，因此重写 `UITableViewCell` 以下两个方法即可：
...
58. `swift 3` 之后，函数有返回值没有被使用的时候会报警告，在该函数前加 `@discardableResult` 可以让编译器不报警告
...
app启动时间、恢复运行时间等时间  动态库链接更耗时会影响app正常启动的问题 
