...
优化前，只有post的时间，大概是 平均在1.5

优化后，postmain 平均 700ms  90分位 1.3s   premain 700ms

动态库  减少400ms左右  1.2 - 800
+load、静态优化 70ms
...
2、启动任务杂乱添加，无规范
3、wnsModule 初始化耗时大
4、load、静态函数的乱用，pageIn的耗时累计
5、闪屏、安全模式、是否首次安装区分

- 用户体验优化
  - 首页展示耗时，无效加载多（用户首页交友tab、关注的加载，其他tab的加载）
  - 闪屏页和rootNav同时构建，


- 在组件化中已处理的部分
    - 全工程组件化，三方库、基础库静态库
- 无用类、图片的清理（流水线脚本扫描，codecc静态扫描）
  - 扫Mach-o的__DATA __objc_selrefs和__TEXT __objc_methname
  - +load方法的梳理  100
...
- 能滞后的SDK初始化都滞后
- 能减少的IO 操作都减少（userdefault、plist读取、db、IO）
- 减少配置项读取，本身就耗时还依赖接口
- 预加载启动图，特别是新版本启动图，最后裁剪处理，别用整张大图，特别是白底那种更可以裁剪处理
...
- 滞后非必要+load方法的调用，放在低优先级启动任务注册的地方调用
- 静态变量非必要都移动到自己方法内
- 减少动态库，除非必要不用动态库
- 无用代码清理，外网的无用业务这一块还没清理
- page in重命名和二进制重排的还没有处理
...
- 利用RunLoop空闲时间执行预缓存任务  FDTemplateLayoutCell
...
#### http和https的区别，说一下http和https的请求过程？
...
AutoreleasePool并没有单独的结构，而是由若干个AutoreleasePoolPage作为结点以双向链表的形式组合而成，会在一个Page空间占满时进行增加
...
```
多线程模型你肯定做过比较选择。死锁是啥肯定也是要知道的，没遇到过至少能举个简单的例子来说明。
...
在建立全文索引时主要通过分词器（lexer），将被索引的表上的那一列的所有值（即文本，要varchar等类型）分割成一个个token_text（比如，英文分词器BASIC_LEXER以空格，标点符号，_，-，等非字母数字的符号作为分割标记），而且记录这些token_text来自表里的哪些行等信息，最后将这些token_text和这些信息都写到表DR$索引名$I中。有了这张表，全文索引才可以快速地查找（无论是模糊还是不模糊查找），但是该表要占据空间的。
```
...
####import 跟 #include 有什么区别，@class呢，#import<> 跟 #import”” 有什么区别？
...
```
抽象和封装，方便使用。首先是对问题有充分的了解，比如构建一个文件解压压缩框架，从使用者的角度出发，只需关注发送给框架一个解压请求，框架完成复杂文件的解压操作，并且在适当的时候通知给使用者，如解压完成、解压出错等。在框架内部去构建对象的关系，通过抽象让其更为健壮、便于更改。其次是API的说明文档。
