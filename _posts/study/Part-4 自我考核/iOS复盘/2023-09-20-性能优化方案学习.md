...
渲染好的位图存放在FrameBuffer里面，如果切换的时候 新的还没绘制完，会导致展示一半，产生屏幕撕裂错觉

所以苹果新增一个 backBuffer，VSync切换的时候 是切换FrameBuffer 到已经准备好的 backBuffer，
所以当VSync切换的时候，缓冲池中的frame可以立马展示，解决屏幕撕裂问题

但是如果VSync切换的时候，backBuffer并没有渲染完成，就不会发生更换，产生卡住的表现（掉帧）
...
CALayer 本质是一个纹理，CPU渲染的基础，设备刷新时，CALayer读取生成好的bitmap展示

方式一：content image 

直接放位图，比如把图片给layer展示  或者异步绘制好图片 再添加到layer

方式二：custom drawing

- CALayer 尝试 - (void)displayLayer:(CALayer *)layer;

- 如果没有实现上面方法 就尝试 绘制，UIView的 -drawRect: 本质上是Layer在绘制：
- (void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx;（ctx包括绘制的空白bitmap、绘制上下文信息）

- 最后，由 Core Graphics 绘制生成的寄宿图会存入 backing store



问题：如果我们变更了UI 如何主动通知重新绘制渲染，如果不主动通知，什么时候会重新绘制渲染

子类变化，主动父类设置标记


setNeedsLayout: 脏标记，下一个运行循环休眠前统一处理

layoutIfNeeded: 立即执行，较好的验证就是当约束赋值之后，setNeedsLayout并不会改变frame，但是 layoutIfNeeded 会
...
树结构：UIView 视图树，CALayer 图层树（呈现树、渲染树）

绘制好了，渲染是在Render Server
...
iOS 堆栈符号解析  https://juejin.cn/post/6844903598011187213
