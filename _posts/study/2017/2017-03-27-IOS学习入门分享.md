...
第二步、添加一个普通控制器场景A，把它作为导航条控制器的  rootViewController

第三步、再添加一个普通控制器场景B，并从A拉出一条转场线链接到B场景，设置转场的ID：segueId
...
到此，我们就可以通过代码以转场的方式从A场景过渡到B场景。

```
self.performSegue(withIdentifier: "segueId", sender: nil)
```

或者直接通过绑定按钮事件的方式调起转场，后面UI控件里面再讲。
