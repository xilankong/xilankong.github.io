...
- #### @符号的使用
  ...
  @end //结束标示，类的声明实现，协议的声明等的标签都是成对出现。
...
- #### 断点调试
  ...
  2.断点调试区域也是大多数IDE 都有的调试方式，进入断点，下一步、跳入、跳出

  ![](https://xilankong.github.io/resource/da_debug_bar.png)

  ​

  3.视图调试 ：上图中的Debug view graph 用于视图调试
...
- #### storyboard & xib
  ...
  然后再代码中通过 segueId 进行转场

  ```
  self.performSegue(withIdentifier: segueId, sender: nil)
  ```

  松开鼠标后，在弹出菜单中 选择 Manual Segue 中的选项，这里选择 Show。
...
第四步、再添加一个普通控制器场景B，并从A中按钮拉出一条转场线链接到B，设置转场的ID：segueId（按住CTRL键，点击按钮拖拽出一条线连接到目标控制器范围内连出segue线。）
