...
![](https://xilankong.github.io/resource/xcodeIDE.jpeg)
...
调试区断点工具栏，配合上导航区的调试导航一起进行断点调试

![](https://xilankong.github.io/resource/debugtoolbar.png)

#### 2.视图调试

调试区工具栏中得视图调试按钮

![](https://xilankong.github.io/resource/uitest.png)

视图调试可以层次分明的看到当前控制器的结构、并且在导航区有对应的类层次结构。

![](https://xilankong.github.io/resource/uixiaoguo.png)
...
![](https://xilankong.github.io/resource/storyboard_ide.jpeg)

#### 认识场景和起始标签

1.起始标签指定的控制器就是app启动后的第一个页面。

2.场景可以通过上图中的右侧工具区的视图对象列表中拖拽到视图编辑区域生成。

![](https://xilankong.github.io/resource/storyboard.png)

#### 两个场景之间设置转场

按住CTRL键，由上图的场景视图的第一个元素，View Controller 对应的小圆点处点击拖拽出一条线连接到目标控制器范围内松开鼠标。

![](https://xilankong.github.io/resource/segueline.png)

在弹出弹窗中 选择Manual Segue中的选项即可连接成功

![](https://xilankong.github.io/resource/seguelinepop.jpeg)

选中连好的线，并在右边工具区 show the Attributes Inspector 下设置好segueid，那么到此就成功创建一个转场。

![](https://xilankong.github.io/resource/segueid.png)

#### 跳转Demo

熟悉了基本创建后，我们实现一个简单的demo，demo包括A、B两个页面、A到B的跳转。

一般使用系统的导航条会更加的方便

第一步、添加一个UINavigationController（导航条控制器）的场景，然后为它添加上起始标签。

第二步、添加一个普通控制器场景A，并用拉转场线一样的方式，把它作为导航条控制器的  rootViewController 设置为 RelationShip segue。

第三步、在A中添加一个按钮

第四步、再添加一个普通控制器场景B，并从A中按钮拉出一条转场线链接到B场景，设置转场的ID：segueId（按住CTRL键，点击按钮拖拽出一条线连接到目标控制器范围内连出转场线。）

如下图：

![](https://xilankong.github.io/resource/storyboard_segue.png)



到此，我们启动app，就可以通过点击按钮从A跳转到B了。

或者直接通过代码绑定事件的方式调起转场，后面再说。
