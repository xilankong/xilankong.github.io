...
1、let 属性 可以在初始化的时候再赋值，因为初始化只会执行一次
...
OC中用来告诉调用者要用这个方法去初始化对象

https://www.cnblogs.com/smileEvday/p/designated_initializer.html

如果swift继承自oc，1、实现父类的指定构造器 2、如果实现了自己的指定构造器，指定构造器需要调用父类指定构造器，同时会继承父类的指定构造器

```
例如 继承自UIView 但是不调用UIView的指定构造器会有错误提示：
...
一个表达式 json["num"] as? String 解析成功，我们就绑定这个结果，
guard let result = json["num"] as? String ,  
同时考虑其他情况，

guard let result = json["num"] as? String ,let num = Double(result) else {
    return 
}
...
**摘要：**Swift中表示“类型范围作用域”这一概念有着两个不同的关键字，分别为static和class，而在OC等其他语言中，开发者不会特地区分变量/类方法和静态变量/静态函数。
...
```
 #selector(YangSliderMenuViewController.buttonClickAction) 
```
...
http://www.cnblogs.com/guoxiaoqian/tag/Swift/
...
Swift 4.0以后的swizzing只能在Appdelegate里面主动调用了
...
Swift 4.0后 字符串提供了一些直接把字符串当数组处理的方法
...
Swift4的String，具备了Sequence 和Collection的所有性质.
```
