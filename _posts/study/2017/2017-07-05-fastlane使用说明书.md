...
fastlane 会要求填写你的Apple ID，选择你的Team(如果有多个) 然后fastlane会自动检测当前目录下项目的App Name和App Identifier、Project。然后自行确认并按流程执行。
...
1.当第一次使用fastlane安装的时候，会要求输入账号密码核实你的身份来连接ADC，这个时候你提供的登录验证会处理为会话存到 spaceship 的 cookie，会话大概一个月有效期，一个月后失效，通常我们只有在打包失败后才会发现这个问题。
2.如果开启了两部验证的AppID 怎么办？

Q1：通过CI去提交包到TestFlight/iTunes Connect 的时候，需要登录iTunes Connect，为了全自动化，需要我们生成一个特定于应用程序的密码:
...
5.通过环境变量FASTLANE_SESSION提供session cookies。
...
#### 3.关于iOS11新两步验证对 Jenkins 自动化出包和提包的影响

iOS11升级以后出现一个两步验证的问题，如果没有使用过，或者会话过期都会出现。之前是只需要输入手机弹出的4位验证码，现在要求点击允许并输入验证码。下面是正常允许并输入验证码后的提示。(后面还会解释这个问题)
...
不是配置在fastfile文件中，而是配置在系统文件中，打包机.bash_profile 或者jenkins环境变量
```
...
##### 1、新建一个fastlane需要选择模式

```
一般选择 Manual setup

并且，好像需要自己配置Appfile了（之前会要求输入自动配置）

```

##### 2、fastlane gym produces error: method `to_plist' not defined in Array #11503
