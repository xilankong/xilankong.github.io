...
当label被压缩时 需要手写的 收缩扩展优先级的设置

xib默认view设置的优先级是750  250  如果需要label不会被遮盖，则必须是最高优先级 防止压缩。
...
抗压缩优先级 Content Compression Resistance  优先级越大 越不会被压缩   默认优先 左  上。   默认情况下两边的label的Content Hugging和Content Compression优先级都是一样的



当一个View 高度需要根据内容来扩张，那么内部label 和 view的优先级怎么判断 有什么不同吗？

当我们需要保证View中得内容都完全显示 并正常撑大View 需要注意内部组件的抗压缩优先级。
